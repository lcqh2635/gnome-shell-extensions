...
# 源代码仓库 https://github.com/WSID/gnome-shell-screencast-extra-feature
# git clone --depth=1 https://github.com/WSID/gnome-shell-screencast-extra-feature.git
# ==============================================================================
...
%global uuid screencast.extra.feature@wissle.me

# ==============================================================================
# 2. 包基本信息 (Header)
# ==============================================================================
# 包的名称。通常与扩展名或项目名一致。
Name:           gnome-shell-extension-screencast-extra-feature
# 版本号。
...
Release:        1%{?dist}
# 简短描述。出现在软件中心的列表中。
Summary:        Extra feature for built-in screencast.
# 许可证类型。必须与源码中的 LICENSE 文件一致。
License:        GPL-3.0-or-later
# 项目主页 URL。
URL:            https://github.com/WSID/gnome-shell-screencast-extra-feature
# 源代码压缩包。可以指向 GitHub 的 Release 或直接使用克隆的源码
# 方式1：指向 Release (推荐)
# 这里假设源码是以 Zip 包形式发布，且文件名包含 UUID
# 源码：zip 包（GNOME 扩展通常是纯脚本，无需编译）
# https://github.com/WSID/gnome-shell-screencast-extra-feature/archive/refs/heads/main.zip
...
# ==============================================================================
%description
Add extra features in built-in screen cast feature. Record Audio with it. 
Specify framerate. (Up to 60 Hz) Downsize resolution. (75%, 50% or 33%) Quick Stop by pressing screen cast key. (Usually Ctrl + Alt + Shift + R)
...
# ------------------------------------------------------------------------------
%prep
# 在 ~/rpmbuild/BUILD 目录下创建 screencast.extra.feature@wissle.me/ 并进入
# %{_builddir}		~/rpmbuild/BUILD		RPM 构建的根目录
# %{buildsubdir}	%{name}-%{version}-build	由 %mkbuilddir 宏设置，用于构建隔离
# %{uuid}		screencast.extra.feature@wissle.me	你自定义的扩展 UUID
# -c：在当前目录（即 %{_builddir}/%{buildsubdir}）创建新目录 %{uuid}
# -n "%{uuid}"：指定新目录的名称为 screencast.extra.feature@wissle.me
# 🔑 关键：-T 跳过 rpmuncompress，避免 tar 误解压 zip
# 自动 cd：RPM 会自动 cd 进入这个新目录，后续 %build/%install 都在此执行
%setup -q -c -n "%{uuid}" -T
# 2. 将扁平压缩包解压到当前目录（即 %{uuid}）
# 解压后产生嵌套：screencast.extra.feature@wissle.me/gnome-shell-screencast-extra-feature-main/
unzip -q -o %{SOURCE0} -d .
...
# ==============================================================================
# 1. 将 spec 文件放到正确位置
# cp gnome-shell-extension-screencast-extra-feature.spec ~/rpmbuild/SPECS/
# 2. 进入 SPECS 目录
# cd ~/rpmbuild/SPECS/
# 🔍 检查 spec 语法
# rpmlint ~/rpmbuild/SPECS/gnome-shell-extension-screencast-extra-feature.spec
# 3. 下载源码到 SOURCES（spectool 会自动处理 Source0/1/2）
# spectool -g -R gnome-shell-extension-screencast-extra-feature.spec
# ✅ 验证源码是否下载成功
# ls -lh ~/rpmbuild/SOURCES/ | grep add-to-desktop
# 4. 生成 SRPM（源码 RPM）
# rpmbuild -bs gnome-shell-extension-screencast-extra-feature.spec
# ✅ 查看生成的 SRPM
# ls -lh ~/rpmbuild/SRPMS/
# 5. 直接生成本地 RPM
# rpmbuild -bb gnome-shell-extension-screencast-extra-feature.spec
# 生成的 RPM 位置
# ls -lh ~/rpmbuild/RPMS/noarch/
# 安装测试
# sudo dnf install -y ~/rpmbuild/RPMS/noarch/gnome-shell-extension-screencast-extra-feature-*.fc44.noarch.rpm
# sudo dnf remove -y gnome-shell-extension-screencast-extra-feature
