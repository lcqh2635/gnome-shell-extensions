...
# 源代码仓库 https://gitlab.com/p91paul/status-area-horizontal-spacing-gnome-shell-extension
...
%global uuid status-area-horizontal-spacing@mathematical.coffee.gmail.com
...
# ------------------------------------------------------------------------------
%prep
# 在 ~/rpmbuild/BUILD 目录下创建 status-area-horizontal-spacing@mathematical.coffee.gmail.com/ 并进入
# %{_builddir}		~/rpmbuild/BUILD		RPM 构建的根目录
# %{buildsubdir}	%{name}-%{version}-build	由 %mkbuilddir 宏设置，用于构建隔离
# %{uuid}		status-area-horizontal-spacing@mathematical.coffee.gmail.com	你自定义的扩展 UUID
# -c：在当前目录（即 %{_builddir}/%{buildsubdir}）创建新目录 %{uuid}
# -n "%{uuid}"：指定新目录的名称为 status-area-horizontal-spacing@mathematical.coffee.gmail.com
# 🔑 关键：-T 跳过 rpmuncompress，避免 tar 误解压 zip
# 自动 cd：RPM 会自动 cd 进入这个新目录，后续 %build/%install 都在此执行
%setup -q -c -n "%{uuid}" -T
# 2. 将扁平压缩包解压到当前目录（即 %{uuid}）
# 解压后产生嵌套：status-area-horizontal-spacing@mathematical.coffee.gmail.com/status-area-horizontal-spacing-gnome-shell-extension-master/
unzip -q -o %{SOURCE0} -d .
...
echo "编译阶段：开始编译源代码..."
cd status-area-horizontal-spacing-gnome-shell-extension-master
cp -r -p %{uuid}/* ..
cd ..
rm -rf status-area-horizontal-spacing-gnome-shell-extension-master
...
# ==============================================================================
# 1. 将 spec 文件放到正确位置
# cp gnome-shell-extension-status-area-horizontal-spacing.spec ~/rpmbuild/SPECS/
# 2. 进入 SPECS 目录
# cd ~/rpmbuild/SPECS/
# 🔍 检查 spec 语法
# rpmlint ~/rpmbuild/SPECS/gnome-shell-extension-status-area-horizontal-spacing.spec
# 3. 下载源码到 SOURCES（spectool 会自动处理 Source0/1/2）
# spectool -g -R gnome-shell-extension-status-area-horizontal-spacing.spec
# ✅ 验证源码是否下载成功
# ls -lh ~/rpmbuild/SOURCES/ | grep add-to-desktop
# 4. 生成 SRPM（源码 RPM）
# rpmbuild -bs gnome-shell-extension-status-area-horizontal-spacing.spec
# ✅ 查看生成的 SRPM
# ls -lh ~/rpmbuild/SRPMS/
# 5. 直接生成本地 RPM
# rpmbuild -bb gnome-shell-extension-status-area-horizontal-spacing.spec
# 生成的 RPM 位置
# ls -lh ~/rpmbuild/RPMS/noarch/
# 安装测试
# sudo dnf install -y ~/rpmbuild/RPMS/noarch/gnome-shell-extension-status-area-horizontal-spacing-*.fc44.noarch.rpm
# sudo dnf remove -y gnome-shell-extension-status-area-horizontal-spacing
