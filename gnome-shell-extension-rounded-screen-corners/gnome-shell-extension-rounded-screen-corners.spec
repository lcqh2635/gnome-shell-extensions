...
# 源代码仓库 https://github.com/lennart-k/gnome-rounded-corners
# git clone --depth=1 https://github.com/lennart-k/gnome-rounded-corners.git
# ==============================================================================
...
# ==============================================================================
# 包的名称。通常与扩展名或项目名一致。
Name:           gnome-shell-extension-rounded-screen-corners
# 版本号。
...
Release:        1%{?dist}
# 简短描述。出现在软件中心的列表中。
Summary:        The most useful GNOME extension on the planet. On OLED screens save energy and money by keeping pixels dark.
...
# https://github.com/lennart-k/gnome-rounded-corners/archive/refs/heads/master.zip
...
echo "编译阶段：开始编译源代码..."
cd gnome-rounded-corners-master
make
unzip %{uuid}.zip -d ..
cd ..
rm -rf gnome-rounded-corners-master
...
# ==============================================================================
# 1. 将 spec 文件放到正确位置
# cp gnome-shell-extension-rounded-screen-corners.spec ~/rpmbuild/SPECS/
# 2. 进入 SPECS 目录
# cd ~/rpmbuild/SPECS/
# 🔍 检查 spec 语法
# rpmlint ~/rpmbuild/SPECS/gnome-shell-extension-rounded-screen-corners.spec
# 3. 下载源码到 SOURCES（spectool 会自动处理 Source0/1/2）
# spectool -g -R gnome-shell-extension-rounded-screen-corners.spec
# ✅ 验证源码是否下载成功
# ls -lh ~/rpmbuild/SOURCES/ | grep add-to-desktop
# 4. 生成 SRPM（源码 RPM）
# rpmbuild -bs gnome-shell-extension-rounded-screen-corners.spec
# ✅ 查看生成的 SRPM
# ls -lh ~/rpmbuild/SRPMS/
# 5. 直接生成本地 RPM
# rpmbuild -bb gnome-shell-extension-rounded-screen-corners.spec
# 生成的 RPM 位置
# ls -lh ~/rpmbuild/RPMS/noarch/
# 安装测试
# sudo dnf install -y ~/rpmbuild/RPMS/noarch/gnome-shell-extension-rounded-screen-corners-*.fc44.noarch.rpm
# sudo dnf remove -y gnome-shell-extension-rounded-screen-corners
